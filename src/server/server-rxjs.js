...
const { catchError, concatMap, map, toArray } = require("rxjs/operators");
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(siteUrl, requestOptions, (response) => {
      if (
        ...
        fetchTitle(new URL(response.headers.location, siteUrl).href)
          .then(resolve)
          .catch(reject);
        return;
      }
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && parsedUrl.pathname === "/I/want/title/") {
    ...
    // Create an observable from URLs
    from(urls)
      .pipe(
        concatMap((siteUrl) =>
          from(fetchTitle(siteUrl)).pipe(
            catchError(() =>
              of(
                `<li><a href="${normalizeUrl(siteUrl)}">${normalizeUrl(
                  siteUrl
                )}</a> - NO RESPONSE</li>`
              )
            )
